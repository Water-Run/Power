...
当前主线已迁至 C# 与 Unity 3D。本文件描述迁移前的原型，参见 [存档说明](ARCHIVE.md)与[当前 README](../../README.md)。
...
本轮架构、实际物理范围、求解方程、实验格式和未来扩展路线见 [可组合模型运行时](docs/MODEL_RUNTIME.md)。更强的基础模型通过结构化定义、测试与实验参与开发；仿真运行时使用已编译模型，保持确定性与独立部署。
...
- [项目命名约定](docs/NAMING.md)
- [上游源码测绘](docs/UPSTREAM_SURVEY.md)
- [目标架构与模型范围](docs/ARCHITECTURE.md)
- [可组合模型运行时与未来模型开发架构](docs/MODEL_RUNTIME.md)
- [首阶段实现方案](docs/IMPLEMENTATION.md)
- [当前开发状态与整机集成基线](docs/DEVELOPMENT_STATUS.md)
- [LuaInstaller 与平台发行设计](docs/PACKAGING.md)
- [验证与性能基线](docs/VALIDATION.md)
- [阶段路线图](docs/ROADMAP.md)
- [需求追踪与规划完成审计](docs/TRACEABILITY.md)
- [上游版本与许可边界](reference/README.md)

## 首批完整动力总成样例

- [EA211 DJS + DQ200](assets/samples/ea211_djs_dq200/README.md)：候选基线为上汽大众 DJS + 0CW.C，已建立完整系统边界、证据清单和待测参数；尚无可发布标定。
- [PSA EC5 + AT8 4AT](assets/samples/psa_ec5_at8_4at/README.md)：候选基线为 C-Elysée 培训资料中的 EC5 115 Ch + 四挡 BVA AT8，已覆盖变矩器、液压/行星机构、热与电控取证边界；不要与后来的八挡 EAT8 混淆。
