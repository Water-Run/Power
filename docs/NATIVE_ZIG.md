...
they do not require C source or headers in this repository. The C# P/Invoke
experiment runner in the .NET build tool remains a consumer of this boundary.
Native engine,
...
model operations use the existing C#/JSON and Zig hosts plus the C# build
tool's P/Invoke ABI host. Lua proposals
...
The compiler is pinned to Zig 0.15.2 in `.zig-version`. The build tool's
`install-zig` command (inside `tools/Build.cs`) uses the official
...
tooling disables Apple SDK discovery only in its Zig build subprocesses by
...
The `native-verify` implementation (inside `tools/Build.cs`) rejects C/C++
source and headers plus Lua source, bytecode and packages. It checks the
compiler pin and formatting, builds the library and both Zig hosts, runs the
Zig suites and the C# P/Invoke ABI host suite, and compares the electrothermal
