...
**2026-09-14: Core gas-network checkpoint.** Fixed-volume gas chambers, reservoir restrictions, gas-to-wall heat links and transactional conservation ledgers now pass analytic and rollback checks. See [the checkpoint boundary](GAS_NETWORK.md). Portable gas assets and CLI/MCP/Unity integration are the next step; the engine milestone remains open.
...
| 5. 发动机物理 | 曲柄连杆、容积与质量/能量、进排气、燃烧、泵气、壁面传热、气缸循环 | C# sealed adiabatic cylinder and slider-crank implemented; compiled finite gas volumes, restrictions and thermal links validated through the [Core gas network](GAS_NETWORK.md); portable model integration remains open; full engine cycle remains open |
...
The [gas-exchange primitives](GAS_EXCHANGE.md) and compiled gas-network checks now run
against both Core target assemblies. The next integration slice must carry the same
parameters and input rules through JSON, portable assets, MCP and replay before adding
Unity gas views. Moving-cylinder pressure work remains a separate solver increment.
